...
```bash
wget https://github.com/Pas-Kapli/mptp/releases/download/v0.2.1/mptp-src-0.2.1.tar.gz
tar zxvf mptp-src-0.2.1.tar.gz
cd mptp-src-0.2.1
./configure
make
make install  # as root, or run sudo make install
...
```bash
wget https://github.com/Pas-Kapli/mptp/releases/download/v0.2.1/mptp-0.2.1-linux-x86_64.tar.gz
tar zxvf mptp-0.2.1-linux-x86_64.tar.gz
```

You will now have the binary distribution in a folder called
`mptp-0.2.1-linux-x86_64` in which you will find three subfolders `bin`, `man`
